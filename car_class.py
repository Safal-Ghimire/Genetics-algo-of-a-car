...
import pygame
import numpy
from common import screen ,vision_threshold,ray_count
from functions import distance,relu,mutate_weights,render_text


class Car():
    def __init__(self, pos,angle,parent_status=None,):
        self.pos = list(pos)
        ...
        if parent_status is None: #intitalizing the weights of the first generation
            ...
            self.w_ih1 = mutate_weights(parent_status.w_ih1, self.mutation_rate,self.mutation_strength)
            self.w_h1h2 = mutate_weights(parent_status.w_h1h2, self.mutation_rate,self.mutation_strength)
            self.w_h2o = mutate_weights(parent_status.w_h2o, self.mutation_rate,self.mutation_strength)
