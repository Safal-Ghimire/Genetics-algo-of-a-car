...
from functions import distance,create_road_body,render_text,fill_empty_spaces,draw_before_updating,get_road_body_center,draw_finish_point,calculate_points,get_max_index,calculate_angle,split_list
...
while running:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if event.button==3: #listen for right click ? #i can retouch this not necessary tho
                erase= not erase
        ...
        for i in  range (20):
            if car[i].coolision==False:
                ...
                if draw_session==0:
                    ...
                    car[i].pos=list(cord)#new center of car is of the position of mouse
            ...
            for unfiltered_road_cord_bloak in all_road:
                ...
                for road_cord in unfiltered_road_cord_bloak:
                    if cord_rect.collidepoint(road_cord[0], road_cord[1]):
                        road_cord_block=split_list(unfiltered_road_cord_bloak,road_cord)
                        collision= True
...
#all car logics
    ...
    if frame==int(600/n):
        for i in range(20):
            ...
            car_points.append(calculate_points(car_cord,finish_point,car[i].pos,car[i].total_forward)-car[i].total_backward)
        frame=0
        generation+=1
        index=sorted(range(len(car_points)), key=lambda i: car_points[i], reverse=True)[:3]
