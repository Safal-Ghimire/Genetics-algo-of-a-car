...
def mutate_weights(weights, mutation_rate,mutation_strength):
        ...
        mask = (numpy.random.random(weights.shape) < mutation_rate).astype(int)#first generate a random probability score between 0 and 1 if it is less then the mutation rate or (in simple terms probability) set it to 1 else 0 it generate a random true event based on the probability given
